...
theme_black <- function(){
  theme(
    ...
    plot.title=element_text(size=12,colour="gray55"),
    axis.title=element_text(size=10,colour="gray55"),
    axis.text=element_text(size=8,colour="gray55",margin=margin(t=1,r=1,b=1,l=1)),
    ...
    legend.text=element_text(size=10,colour="gray55"),
    legend.key.size=unit(.75,'lines'),
    strip.background=element_blank(),
    strip.text=element_text(size=10,colour="gray55",face="bold",margin=margin(.1,0,.1,0,"cm"))
  )
}
...
datasub_dt$actor <- factor(datasub_dt$actor,levels=unique(datasub_dt$actor),labels=c("State forces","Rebel groups","Political militas","Identity militas","Militias"))
...
# 
# ### polit only
# gg_politmap <- ggplot(data = africa) +
#   geom_sf(color="gray65",fill=NA,size=.25)+
#   geom_point(data=datasub_dt[actor=="Political militas"],aes(x=longitude,y=latitude,size=incidents),color="steelblue")+
#   scale_size(range=c(.2,2.8))+
#   coord_sf(ylim=c(-35,37))+
#   labs(title="Political militias")+
#   theme_void()+
#   theme_white()+
...
# 
# datasubc_dt <- datacomb_dt[,.(incidents=sum(incidents)),by=.(longitude,latitude)]
# datasubc_dt <- datasubc_dt[incidents>=1]
# 
# ### all actors
# gg_combmap <- ggplot(data = africa) +
#   geom_sf(color="gray65",fill=NA,size=.25)+
#   geom_point(data=datasubc_dt,aes(x=longitude,y=latitude,size=incidents),color="gray30")+
#   scale_size(range=c(.2,2.8))+
#   coord_sf(ylim=c(-35,37))+
#   labs(title="All actors")+
#   theme_void()+
#   theme_white()+
#   theme(axis.line.x=element_blank(),axis.line.y=element_blank(),axis.title = element_blank(),axis.text = element_blank(),legend.position = c(.30,.30),legend.text = element_text(hjust=1))
# 
# 
# ggsave("Figures/map_violence_polit.png",gg_politmap,width=6.5,height=6.5,dpi=200)
# ggsave("Figures/map_violence_combined.png",gg_combmap,width=6.5,height=6.5,dpi=200)
...
gg_tsincidents_black <- ggplot(moset_dt[actor!="milit"],aes(x=date,y=incidents,color=actor,linetype=actor))+
  geom_line(size=.5) +
  scale_color_manual(values=c("gray20","black","gray20","black"),labels=c("State forces","Rebel groups","Political militias","Identity militias"))+
  scale_linetype_manual(values=c(5,2,1,3),labels=c("State forces","Rebel groups","Political militias","Identity militias"))+
...
gg_tsincidents_white <- ggplot(moset_dt[actor!="milit"],aes(x=date,y=incidents,color=actor,linetype=actor))+
  ...
  scale_linetype_manual(values=c(1,2,5,3),labels=c("State forces","Rebel groups","Political militias","Identity militias"))+
  labs(x="Year",y="Incidents")+
  theme_classic()+
  theme_white()+
  theme(legend.position = c(.12,.88))
...
# gg_coverage_black <- ggplot(moset3_dt,aes(x=date,y=country))+
#   geom_area(stat="identity",fill="darkgray")+
#   labs(x="Year",y="Countries")+
#   theme_classic()+
#   theme_black()
# 
# gg_coverage_white <- ggplot(moset3_dt,aes(x=date,y=country))+
#   geom_area(stat="identity",fill="darkgray")+
#   labs(x="Year",y="Countries")+
#   theme_classic()+
#   theme_white()
# 
# gg_incidents_coverage_black <- plot_grid(gg_tsincidents_black,gg_coverage_black,ncol=1,align="v",axis="b",rel_heights = c(4,1))
# 
# gg_incidents_coverage_white <- plot_grid(gg_tsincidents_white,gg_coverage_white,ncol=1,align="v",axis="b",rel_heights = c(4,1))
...
# ## separate series
# 
# datasubp_dt <- moset_dt[actor!="milit"]
# datasubp_dt[,`:=`(polit=ifelse(actor=="polit","Political militias","All other groups"))]
# datasubp_dt <- datasubp_dt[,.(incidents=sum(incidents)),by=.(year,date,polit)]
# datasubp_dt$polit <- factor(datasubp_dt$polit,levels=unique(datasubp_dt$polit)[c(2,1)])
# 
# # histogram of incidents by grid-cells
# gg_tspolit <- ggplot(datasubp_dt,aes(x=date,y=incidents,color=polit,linetype=polit))+
#   geom_line(size=.6) +
#   scale_color_manual(values=c("steelblue","gray30"),labels=c("Political militias","All other groups"))+
#   scale_linetype_manual(values=c(1,2),labels=c("Political militias","All other groups"))+
#   labs(x="Year",y="Incidents")+
#   theme_classic()+
#   theme_white()+
#   theme(legend.position = c(.12,.88))
# 
# 
# ggsave("Figures/series_violence_polit.png",gg_tspolit,width=6.5,height=3.5,dpi=200)
# ggsave("Figures/series_violence_polit.eps",gg_tspolit,width=6.5,height=3.5,dpi="retina",device=cairo_ps)
...
crops_dt <- crops_dt[,.(x,y,Crop,Max_area,Maize_area,Sorghum_area,Wheat_area=Wheat.Winter_area,Rice_area)] 
crops_dt[Crop=="Wheat.Winter"]$Crop <- "Wheat"

crops_dt$latitude <- as.numeric(crops_dt$y)
crops_dt$longitude <- as.numeric(crops_dt$x)

crops_dt[,`:=` (Tot_area=Maize_area+Sorghum_area+Wheat_area+Rice_area)] 
crops_dt[is.na(crops_dt)] <- 0
crops_dt[,`:=` (Maize_share=Maize_area/Tot_area,Sorghum_share=Sorghum_area/Tot_area,Wheat_share=Wheat_area/Tot_area,Rice_share=Rice_area/Tot_area)] 
crops_dt[is.na(crops_dt)] <- 0

crops_dt[,`:=` (Maize_dum=ifelse((Maize_area==Max_area & Max_area!=0),1,0),Sorghum_dum=ifelse((Sorghum_area==Max_area & Max_area!=0),1,0),Wheat_dum=ifelse((Wheat_area==Max_area & Max_area!=0),1,0),Rice_dum=ifelse((Rice_area==Max_area & Max_area!=0),1,0))] 
...
gg_prices_black <- ggplot(prices_dt,aes(x=date,y=price,color=cereal,linetype=cereal,group=cereal))+
  geom_line(size=.5)+
  scale_color_manual(name="cereal",values=c("gray35","gray35","black","black"))+
  scale_linetype_manual(name="cereal",values=c(1,2,5,3))+
  labs(x="Year",y="Price")+
  theme_classic()+
  theme_white()+
...
seasonal_dt <- dataset_dt[max_area>0 & actor=="milit",.(incidents=mean(incidents),incidents_dum=mean(incidents_dum),incidents_pop=mean(incidents_pop),max_area=mean(max_area),tot_area=mean(tot_area),population_mln=mean(population_mln)),by=.(season)]
seasonal_dt <- seasonal_dt[order(season)]

scale_coef <- 100*sd(dataset_dt[max_area>0 & actor=="milit"]$price_ch)*seasonal_dt$max_area/seasonal_dt$incidents_dum

coef_fe <- feols(incidents_dum~price_ch:max_area:(i(season,keep=1:12))+log(population_mln)  | xy+country^year, dataset_dt[actor=="milit"])
...
gg_coef_black<- ggplot(coeftab_dt[actor=="Militias"],aes(x=season,y=est))+
  geom_ribbon(aes(ymin=est-1.96*se,ymax=est+1.96*se),fill="gray20",alpha=.25)+
...
dataset_dt[max_area>0,`:=`(tr1=quantile(max_area,.5),tr2=quantile(max_area,.8),tr3=quantile(max_area,.9)),by=.(crop)]
dataset_dt[is.na(dataset_dt)] <- 0

dataset_dt[,`:=`(area_dose=as.factor(ifelse(max_area>0 & max_area<=tr1,1,ifelse(max_area>tr1 & max_area<=tr2,2,ifelse(max_area>tr2 & max_area<=tr3,3,ifelse(max_area>tr3,4,0))))))]
...
seasonal_dt <- dataset_dt[max_area>0 & actor=="milit",.(incidents=mean(incidents),incidents_dum=mean(incidents_dum),incidents_pop=mean(incidents_pop),max_area=mean(max_area),tot_area=mean(tot_area),population_mln=mean(population_mln)),by=.(area_dose,season)]
seasonal_dt <- seasonal_dt[order(area_dose,season)]

scale_coef <- 100*sd(dataset_dt[max_area>0 & actor=="milit"]$price_ch)/seasonal_dt$incidents_dum

coef_sum <- summary(dose_fe,vcov=~xy+country^year)
...
coeftab_dt$est <- coeftab_dt$est*scale_coef
coeftab_dt$se <- coeftab_dt$se*scale_coef
coeftab_dt$actor <- as.factor("Political militias")
...
country_dt <- dataset_dt[actor!="milit",.(incidents=sum(incidents),incidents_dum=mean(incidents_dum),max_area=mean(max_area),tot_area=mean(tot_area),population=mean(population),population_mln=mean(population_mln)),by=.(country)]
...
gg_conflict_white <- ggplot(country_dt,aes(x=reorder(countrylab,incidents),y=incidents))+
  geom_bar(stat="identity",fill="indianred",color="gray65",size=.25)+
  coord_flip(ylim=c(0,max(country_dt$incidents)*1.25))+
  labs(x="Countries/Territories (cells with at least one incident)",y="Incidents")+
  theme_classic()+
  theme_white()

ggsave("Figures/bars_violence.png",gg_conflict_white,width=6.5,height=7.5,dpi=200)
ggsave("Figures/bars_violence.eps",gg_conflict_white,width=6.5,height=7.5,dpi="retina",device=cairo_ps)
...
gg_map_white <- ggplot(data = africa) +
  ...
  theme_void()+
  theme_white()+
  theme(axis.line.x=element_blank(),axis.line.y=element_blank(),axis.title = element_blank(),axis.text = element_blank(),legend.position = c(.30,.20),legend.text = element_text(hjust=1),legend.title = element_text(size=10))
...
ggsave("Figures/bars_map_violence.png",gg_rankmap_white,width=6.5,height=6.5,dpi=200)
ggsave("Figures/bars_map_violence.eps",gg_rankmap_white,width=6.5,height=6.5,dpi="retina",device=cairo_ps)
...
datamid_dt <- datacomb_dt[year==2020]
datamid_dt <- datamid_dt[season %in% 1]
datamid_dt <- datamid_dt[,.(xy,longitude,latitude,country,harvest_mid=month,crop,area=max_area,season)]
datamid_dt$season <- NULL
datamid_dt <- unique(datamid_dt)
...
gg_map_white <- ggplot(data = africa) +
  ...
  scale_color_manual(values=fourseasons_col,guide="none")+
  coord_sf(ylim=c(-35,37))+
  theme_void()+
  theme_white()+
...
# 
# # histogram of incidents by grid-cells
# gg_incidents_black <- ggplot(xyset_dt[incidents>0 & actor!="milit"],aes(x=incidents))+
#   geom_bar(aes(fill=actor),position=position_dodge(),color="gray55",alpha=.9,size=.25) +
#   scale_x_binned(n.breaks=11,limits=c(0,55),labels=c(seq(0,50,5),"50+"))+
#   scale_fill_manual(name="Incidents",values=c("goldenrod","indianred","steelblue","forestgreen"))+
#   labs(x="Incidents",y="Cells")+
#   theme_classic()+
#   theme_black()+
#   theme(legend.position=c(.82,.82))
# 
# # histogram of incidents by grid-cells
# gg_incidents_white <- ggplot(xyset_dt[incidents>0 & actor!="milit"],aes(x=incidents))+
#   geom_bar(aes(fill=actor),position=position_dodge(),color="gray65",alpha=.9,size=.25) +
#   scale_x_binned(n.breaks=11,limits=c(0,55),labels=c(seq(0,50,5),"50+"))+
#   scale_fill_manual(name="Incidents",values=c("goldenrod","indianred","steelblue","forestgreen"))+
#   labs(x="Incidents",y="Cells")+
#   theme_classic()+
...
# ggsave("Figures/incident_distribution.eps",gg_incidents_white,width=6.5,height=3.5,dpi="retina",device=cairo_ps)
...
# 
# 
# xydateset_dt <- dataset_dt[,.(incidents=sum(incidents)),by=.(actor,xy,date)]
# 
# xydateset_dt$actor <- factor(xydateset_dt$actor,levels=unique(xydateset_dt$actor),labels=c("State forces","Rebel groups","Political militas","Identity militas","Militias"))
# 
# 
# # histogram of incidents by grid-cells and date
# gg_cellmonth_black <- ggplot(xydateset_dt[incidents>0 & actor!="Militias"],aes(x=incidents,fill=actor))+
#   geom_bar(color="gray55",alpha=.9,size=.25) +
#   scale_x_binned(limits=c(0,11),breaks=c(seq(0,11,1)),labels=c(seq(0,10,1),"10+"))+
#   scale_fill_manual(values=c("gray20","gray20","gray20","gray20"))+
#   facet_wrap(.~actor)+
#   labs(x="Incidents",y=paste("Cell\u2013","Period", sep=""))+
#   theme_classic()+
#   theme_white()
# 
# # histogram of incidents by grid-cells
# gg_cellmonth_white <- ggplot(xydateset_dt[incidents>0 & actor!="Militias"],aes(x=incidents,fill=actor))+
#   geom_bar(color="gray65",alpha=.9,size=.25) +
#   scale_x_binned(limits=c(0,11),breaks=c(seq(0,11,1)),labels=c(seq(0,10,1),"10+"))+
#   scale_fill_manual(values=c("goldenrod","indianred","steelblue","forestgreen"))+
#   facet_wrap(.~actor)+
#   labs(x="Incidents",y=paste("Cell\u2013","Period", sep=""))+
#   theme_classic()+
#   theme_white()
# 
# ggsave("Figures/incident_cellmonth.png",gg_cellmonth_white,width=6.5,height=5.5,dpi=200)
# ggsave("Figures/incident_cellmonth_bw.png",gg_cellmonth_black,width=6.5,height=5.5,dpi="retina")
# ggsave("Figures/incident_cellmonth.eps",gg_cellmonth_white,width=6.5,height=5.5,dpi="retina",device=cairo_ps)
# ggsave("Presentation/incident_cellmonth.png",gg_cellmonth_white,width=6.5,height=5.5,dpi="retina")
# 
# 
# 
# 
...
# 
# 
# ggsave("Figures/voxeu_harvest.png",gg_map_white,width=3.5,height=3.5,dpi="retina")
# ggsave("Figures/voxeu_harvest.eps",gg_map_white,width=3.5,height=3.5,dpi="retina",device=cairo_ps)
# 
# 
# datasrt_dt <- datacomb_dt[year==2020]
# datasrt_dt <- datasrt_dt[season_srt %in% 1]
...
# ggsave("Figures/harvestseasons.eps",gg_hs_white,width=6.5,height=7.5,dpi="retina",device=cairo_ps)
# 
...
# 
# 
# # fourseasons <- colorRampPalette(colors=c("steelblue","forestgreen","tan","indianred","goldenrod","darkgray","steelblue"),interpolate="spline")
# # 
# # fourseasons_col <- fourseasons(13)[1:12]
# # 
# # plot(c(1:12),col=fourseasons_col,pch=16)
# 
