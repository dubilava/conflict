...
# 
# dataset_dt[,`:=`(price_lag=data.table::shift(price_ch,12),price_lead=data.table::shift(price_ch,12,type="lead")),by=.(xy)]
...
## print to latex
etable(harv_fe,cluster=~xy,tex=F,digits=3,digits.stats = 3,title="Main Results")
...
gg_coef <- ggplot(coeftab_dt,aes(x=season,y=est))+
  ...
  geom_hline(yintercept = seq(-8,12,4),color="gray50",size=.3,linetype=3) +
  geom_hline(yintercept = 0,color="gray30",size=.4,linetype=2) +
  scale_x_continuous(breaks = 0:11,labels=c(0:11))+
  scale_y_continuous(breaks = seq(-4,8,4),labels=seq(-4,8,4))+
  coord_polar(start=-pi/3+1)+
  labs(x="Months after harvest",y="Percent change in violence")+
  theme(axis.text=element_text(size=10,margin=margin(t=1,r=1,b=1,l=1),color="black"),axis.title = element_text(size=10),plot.title = element_text(size=12),panel.grid=element_blank(),panel.background = element_blank(),axis.title.y = element_text(hjust=.78),legend.position = "none")

## saving the plot
ggsave("Figures/circular_ucdp.png",gg_coef,width=6.5,height=6.5,device="png",dpi="retina")
